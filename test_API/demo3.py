...
from aiida.engine import workfunction, submit, calcfunction
...
def submit_single_calc(temps):
    ...
    builder.metadata.label = 'Demo_for_temp_{}'.format(int(temps.value))
    builder.metadata.description = 'Test Demo_for_temp_{} for UppASD-AiiDA'.format(
        int(temps.value))
    submit(builder)
